            ...
            while (active)
            {
                ...
                } else
                {
                    ...
                    while (teleportCount < teleports)
                    {
                        ...
                        if (currentTime <= 0f)
                        {
                            pm.Teleport(ec.RandomCell(includeOffLimits: false, includeWithObjects: false, useEntitySafeCell: false).TileTransform.position + Vector3.up * 5f);
                            AudioManager audMan = PrivateDataHelper.GetVariable<AudioManager>(ec, "audMan");
                            audMan.PlaySingle(AssetsHelper.LoadAsset<SoundObject>("Teleport"));
