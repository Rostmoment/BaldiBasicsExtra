...
namespace BBE.Helpers
{
    class AssetsHelper
    {
        ...
        {
            return (from x in Resources.FindObjectsOfTypeAll<T>() where predicate(x) select x).First();
        }
        ...
        public static SoundObject CreateSoundObject(object audio, SoundType type, Color? color = null, float sublength = 1f, string SubtitleKey = "Rost moment")
        {
            AudioClip clip = null;
            ...
            SoundObject result = ObjectCreators.CreateSoundObject(clip, SubtitleKey, type, color ?? Color.white, sublength);
            return result;
        }
