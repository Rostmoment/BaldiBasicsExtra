...
namespace BBE.CustomClasses
{
    ...
    public class FunSetting : MonoBehaviour
    {
        ...
        private void CheckFunSettingsStatusWithouButton(bool val)
        {
            if (val)
            {
                CachedAssets.FunSettings.Where(x => x.NotAllowed(Enum)).Do(x => x.SetWithoutButton(false));
                CachedAssets.FunSettings.Where(x => DependOf(x.Enum)).Do(x => x.SetWithoutButton(true));
                CachedAssets.FunSettings.Where(x => NotAllowed(x.Enum)).Do(x => x.SetWithoutButton(false));
            }
            else
            {
                CachedAssets.FunSettings.Where(x => x.DependOf(Enum)).Do(x => x.SetWithoutButton(false));
            }
        }
        ...
        private void SetWithoutButton(bool val)
        {
            isActive = val;
            CheckFunSettingsStatusWithouButton(val);
        }
        public void ChangeWithoutButton()
        {
            SetWithoutButton(!Value);
        }
