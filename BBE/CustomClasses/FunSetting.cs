...
namespace BBE.CustomClasses
{
    ...
    {
        private static List<FunSetting> all = new List<FunSetting>();
        ...
        public static FunSetting[] MaximumActiveCount
        {
            get
            {
                FunSetting[] allSettings = GetAll().OrderBy(x => x.NotAllowedList.Count).ToArray();
                List<FunSetting> result = new List<FunSetting>();
                foreach (FunSetting funSetting in allSettings)
                {
                    result.Add(funSetting);
                    if (result.IfExists(x => x.NotAllowed(funSetting.Type), out FunSetting fun)) 
                        result.Remove(fun);
                }
                foreach (FunSetting funSetting in allSettings)
                {
                    if (result.Contains(funSetting))
                        continue;
                    if (result.Exists(x => x.NotAllowed(funSetting.Type)))
                        continue;
                    if (result.Exists(x => funSetting.NotAllowed(x.Type)))
                        continue;
                    result.Add(funSetting);
                }
        ...
        /// <summary>
        /// Calls as postfix for <see cref="PlaceholderWinManager.Initialize"/>
        /// </summary>
        /// <param name="placeholderWin">Instance of <see cref="PlaceholderWinManager"/></param>
        public virtual void OnWin(PlaceholderWinManager placeholderWin)
        {

        }
        /// <summary>
        /// Calls as prefix for <see cref="CoreGameManager.ReturnToMenu"/>
        /// </summary>
        ...
        /// <summary>
        /// Calls as postfix for <see cref="PlayerManager.SetInvisible(bool)"/>
        /// </summary>
        /// <param name="pm"><see cref="PlayerManager"/> instance</param>
        /// <param name="value">Value of parameter <see cref="PlayerManager.SetInvisible(bool)"/></param>
        public virtual void OnPlayerInvisible(PlayerManager pm, bool value)
        {
        ...
        {
            return all.Where(x => x != null).ToArray();
        }
        public static FunSetting[] GetAllFromMod(PluginInfo info)
        {
            return all.Where(x => x.pluginInfo == info).ToArray();
        }
        public static bool Exists(FunSettingsType setting)
        {
            return all.Exists(x => x.funSettingEnum == setting);
        }
        ...
        public static bool Exists(string setting, bool localized)
        {
            if (!localized)
            {
                return all.Exists(x => x.funSettingName == setting);
            }
            return all.Exists(x => x.LocalizedName == setting);
        }
        ...
        public static void Remove(string setting, bool localized)
        {
            ...
            else all.RemoveAll(x => x.funSettingName == setting);
        }
        ...
        {
            all.RemoveAll(x => !exceptions.Contains(x.pluginInfo));
        }
        public static void RemoveAllFromMod(PluginInfo info)
        {
            all.RemoveAll(x => x.pluginInfo == info);
        }
        ...
        private void CheckFunSettingsStatus(bool val)
        {
            if (val)
            {
                all.Where(x => x.NotAllowed(funSettingEnum)).Do(x => x.Set(false));
                all.Where(x => DependOf(x.funSettingEnum)).Do(x => x.Set(true));
                all.Where(x => NotAllowed(x.funSettingEnum)).Do(x => x.Set(false));
            }
            else
            {
                all.Where(x => x.DependOf(funSettingEnum)).Do(x => x.Set(false));
            }
        }
