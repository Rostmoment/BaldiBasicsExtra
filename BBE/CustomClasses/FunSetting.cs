...
namespace BBE.CustomClasses
{
    ...
    public class FunSetting : MonoBehaviour
    {
        ...
        private UnityAction ActOnButtonEnabling;
        private UnityAction ActOnButtonDisabling;
        ...
        public string LocalizedName => Singleton<LocalizationManager>.Instance.GetLocalizedText(funSettingName);

        private static CustomFunSettingData CreateData(PluginInfo pluginInfo, string name, FunSettingsType setting, string desc = "", List<FunSettingsType> dependsOf = null, List<FunSettingsType> notAllowedSettings = null, UnityAction OnEnabling = null, UnityAction OnDisabling = null, UnityAction OnHighlight = null, UnityAction ActOnButtonEnabling = null, UnityAction ActOnButtonDisabling = null)
        {
            if (dependsOf.IsNull())
            {
            ...
            return new CustomFunSettingData()
            {
                ...
                ActOnButtonDisabling = ActOnButtonDisabling,
                ActOnButtonEnabling = ActOnButtonEnabling
            };
        }
        public static FunSetting CreateFunSetting(CustomFunSettingData data)
        {
            return CreateFunSetting(data.Plugin, data.Name, data.Type, data.Description, data.Dependies, data.NotAllowed, data.OnEnabling, data.OnDisabling, data.OnHighlight, data.ActOnButtonEnabling, data.ActOnButtonDisabling);
        }
        public static FunSetting CreateFunSetting(PluginInfo pluginInfo, string name, FunSettingsType setting, string description = "", List<FunSettingsType> dependsOf = null, List<FunSettingsType> notAllowedSettings = null, UnityAction OnEnabling = null, UnityAction OnDisabling = null, UnityAction OnHighlight = null, UnityAction ActOnButtonEnabling = null, UnityAction ActOnButtonDisabling = null)
        {
            ...
            res.ActOnButtonDisabling = ActOnButtonDisabling;
            res.ActOnButtonEnabling = ActOnButtonEnabling;
            res.Set(false);
            go.transform.localScale = new Vector3(0.75f, 0.75f, 0.75f);
            CachedAssets.FunSettings.Add(res);
            CustomFunSettingData data = CreateData(pluginInfo, name, setting, description, dependsOf, notAllowedSettings, OnEnabling, OnDisabling, OnHighlight, ActOnButtonEnabling, ActOnButtonDisabling);
            res.data = data;
        ...
        public void Set(bool val)
        {
            ...
            if (val && !ActOnButtonEnabling.IsNull()) ActOnButtonEnabling();
            if (!val && !ActOnButtonDisabling.IsNull()) ActOnButtonDisabling();
        }
