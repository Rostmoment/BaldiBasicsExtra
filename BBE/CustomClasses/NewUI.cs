...
namespace BBE.ExtraContents
{
    ...
    public class NewUI : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            BasePlugin.Instance.asset.Get<Sprite>("MenuArrowSheet_0"), Text.transform.parent, new Vector3(-180f, -150f, z), () => text.SetText(Singleton<LocalizationManager>.Instance.GetLocalizedText("FunSettingsDesk")));
            ...
            BasePlugin.Instance.asset.Get<Sprite>("MenuArrowSheet_1"), Text.transform.parent, new Vector3(180f, -150f, z), () => text.SetText(Singleton<LocalizationManager>.Instance.GetLocalizedText("FunSettingsDesk")));
        ...
        /*void Update()
        {
            ...
            {
                GameLoader gameLoader = AssetsHelper.Find<GameLoader>();
                gameLoader.gameObject.SetActive(true);
                gameLoader.CheckSeed();
                gameLoader.Initialize(1);
                gameLoader.SetMode((int)Mode.Main);
                ElevatorScreen elevator = SceneManager.GetActiveScene().GetRootGameObjects().Where(x => x.name == "ElevatorScreen").First().GetComponent<ElevatorScreen>();
                gameLoader.AssignElevatorScreen(elevator);
                elevator.gameObject.SetActive(true);
                gameLoader.LoadLevel(TrueGlitchMode.newSceneObject);
                elevator.Initialize();
