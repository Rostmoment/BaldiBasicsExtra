...
namespace BBE
{
    [BepInPlugin("rost.moment.baldiplus.extramod", "Baldi's Basics Extra", "2.1.9.1")]
    ...
    public class BasePlugin : BaseUnityPlugin
    {
        ...
        public void RegisterDataToGenerator(string floorName, int floorNumber, CustomLevelObject floorObject)
        {
            ...
            foreach (RoomGroup roomGroup in floorObject.roomGroup)
            {
                foreach (CustomRoomData room in floorData.Rooms)
                {
                    if (room.Type == roomGroup.name)
                    {
                        roomGroup.potentialRooms = roomGroup.potentialRooms.AddToArray(room.WeightedRoomAsset);
                    }
                }
            }
