...
namespace BBE.Patches
{
    ...
    class FunSettingsEffects
    {
        ...
        private static IEnumerable<CodeInstruction> SlowPlayer(IEnumerable<CodeInstruction> instructions)
        {
            ...
            int x = 0;
            List<int> notAllowed = new List<int> { 14, 15, 16, 17 };
            foreach (CodeInstruction codeInstruction in instructions)
            {
                if (!notAllowed.Contains(x))
                {
                    result.Add(codeInstruction);
                }
                if (x == 17)
                {
                    result.Add(new CodeInstruction(OpCodes.Call, AccessTools.Method(typeof(FunSettingsEffects), "SpeedyPatch")));
                }
                x++;
            }
            return result;
