...
namespace BBE.Patches
{
    ...
    class FunSettingsEffects
    {
        ...
        private static void AddEffects(BaseGameManager __instance)
        {
            ...
            {
                if (Singleton<CoreGameManager>.Instance.GetHud(0).gameObject.GetComponent<MirrorMode>().IsNull()) Singleton<CoreGameManager>.Instance.GetHud(0).gameObject.AddComponent<MirrorMode>().Initialize();
            }
            if (FunSettingsType.TimeAttack.IsActive() && __instance.IsNot<PitstopGameManager>())
            {
