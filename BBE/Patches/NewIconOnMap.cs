        ...
        private static void CustomIcons(BaseGameManager __instance, Map map)
        {
            ...
            foreach (MapIcon icon in icons)
            {
                ...
                if (!icon.target.IsNull())
                {
                    ...
                    if (!pickup.IsNull())
                    {
                        List<Items> YTP = new List<Items>() { Items.Points, (Items)CachedAssets.Enums["YTP_Blue"], (Items)CachedAssets.Enums["YTP_Bronze"], (Items)CachedAssets.Enums["YTP_Red"] };
                        if (YTP.Contains(pickup.item.itemType))
                        {
                            icon.spriteRenderer.sprite = AssetsHelper.SpriteFromFile(Path.Combine("Textures", "MapIcons", "Points.png"), 70f);
                        }
                    }
                }
            }
