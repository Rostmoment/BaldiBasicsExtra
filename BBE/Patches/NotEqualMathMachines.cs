...
namespace BBE.Patches
{
    ...
    class NotEqualMathMachines
    {
        ...
        [HarmonyPatch(typeof(MathMachine), nameof(MathMachine.Completed), new Type[] { typeof(int) })]
        [HarmonyPatch(typeof(MathMachine), nameof(MathMachine.Completed), new Type[] { typeof(int), typeof(bool), typeof(Activity) })]
        [HarmonyPostfix]
        private static void FixWrongAnswer(MathMachine __instance)
        {
            if (!machines.Contains(__instance))
                return;
            if (int.TryParse(__instance.answerText.text, out int converted))
            {
                if (converted == __instance.answer)
                {
                    int tmp = converted;
                    if (tmp != 0)
                    {
                        ...
                        tmp--;
                    __instance.answerText.text = tmp.ToString();
                }
            }
        }*/
