...
namespace BBE.NPCs
{
    public class KeyboardGame : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if (Input.inputString.Length > 0)
            {
                ...
                if (char.IsLetter(inputChar) && typerex.possibleSymbols.Contains(char.ToUpper(inputChar)))
                {
                    try
                    {
        ...
        {
            StopAllCoroutines();
            if (done) Singleton<CoreGameManager>.Instance.AddPoints(word.Length * 10, player.playerNumber, true);
    ...
    public class Typerex : NPC
    {
        ...
        private void Setup()
        {
            sprites.Add("Base", AssetsHelper.TextureFromFile("Textures", "NPCs", "Typerex", "BBE_TyperexBase.png").ToSprite(65));
            sprites.Add("Angry", AssetsHelper.TextureFromFile("Textures", "NPCs", "Typerex", "BBE_TyperexAngry.png").ToSprite(65));
            sprites.Add("Close", AssetsHelper.TextureFromFile("Textures", "NPCs", "Typerex", "BBE_TyperexClose.png").ToSprite(65));
            sprites.Add("Happy", AssetsHelper.TextureFromFile("Textures", "NPCs", "Typerex", "BBE_TyperexHappy.png").ToSprite(65));
        }
