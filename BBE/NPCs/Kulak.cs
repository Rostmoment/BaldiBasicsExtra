...
namespace BBE.NPCs
{
    public class Kulak : NPC
    {
        ...
        public void Setup()
        {
            if (getOut.IsNull()) getOut = AssetsHelper.CreateSoundObject("Audio/NPCs/BBE_Kulak_GetOut.wav", SoundType.Voice, AssetsHelper.ColorFromHex("f5de5b"), SubtitleKey: "Kulak_GetOut");
            if (smash.IsNull()) smash = AssetsHelper.CreateSoundObject("Audio/NPCs/BBE_Kulak_Smash.wav", SoundType.Voice, AssetsHelper.ColorFromHex("f5de5b"), SubtitleKey: "Kulak_Smash");
            if (hiya.IsNull()) hiya = AssetsHelper.CreateSoundObject("Audio/NPCs/BBE_Kulak_Hiya.wav", SoundType.Voice, AssetsHelper.ColorFromHex("f5de5b"), SubtitleKey: "Kulak_Hiya");
            if (animator.IsNull()) animator = gameObject.AddComponent<CustomSpriteAnimator>();
            animator.spriteRenderer = this.spriteRenderer[0];
            animator.animations.Add("Angry", new CustomAnimation<Sprite>(6, AssetsHelper.CreateSpriteSheet("Textures/NPCs/Kulak/BBE_KulakAngry.png", 5, 1, 50).ToArray()));
            animator.animations.Add("Idle", new CustomAnimation<Sprite>(6, AssetsHelper.CreateSpriteSheet("Textures/NPCs/Kulak/BBE_KulakAngry.png", 5, 1, 50).ToArray().ReverseAsArray()));
        }
        public void Angry()
        {
        ...
        private IEnumerator Timer(PlayerManager pm)
        {
            float t = 1.70f;
            while (t > 0)
            {
            ...
            CoreGameManager.Instance.EndGame(pm.transform, this.transform, () => StartCoroutine(Singleton<CoreGameManager>.Instance.EndSequence()));
        }
        /*private void Adv_OnBoxingGloveHit(PlayerManager pm)
        {
            Debug.LogError("Test");
            CoreGameManager.Instance.audMan.PlaySingle(AssetsHelper.CreateSoundObject("Audio/NPCs/BBE_Kulak_YouAreFuckingFaggot.wav", SoundType.Voice, AssetsHelper.ColorFromHex("f5de5b"), 1.70f, "АХ ТЫ СУКА ЕБАНАЯ"));
