...
namespace BBE.NPCs.Chess
{
    public class ChessBoard : MonoBehaviour
    {
        ...
        public IEnumerator MakeMove(Move move, float time)
        {
            ...
            if (chessPieces.Exists(x => x.position == move.end, out BaseChessPiece result))
            {
                result.Capture();
            }
        ...
        {
            if (positionSquares.Values.Exists(x => CursorInside(x), out Vector4 result))
            {
                Position position = positionSquares.FindKey(result);
                if (position == selectedTile)
                    return;
                if (selectedTile == null || selectedPiece == null || selectedPiece.Color != puzzle.PlayerColor || position.PieceAtPosition?.Color == selectedPiece?.Color || !selectedPiece.ValidMoves.Exists(x => x == position))
                {
        ...
        private void Update()
        {
            ...
            {
                if (pm.itm.items.Exists(x => x.itemType.Is(ModdedItems.ChessBook)))
                    ShowCorrectMove();
            }
        }
