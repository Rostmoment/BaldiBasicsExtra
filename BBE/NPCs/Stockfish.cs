...
namespace BBE.NPCs
{
    public class Stockfish : NPC, INPCPrefab
    {
        ...
        public void OldEndGame(PlayerManager pm, ChessBoard chessBoard, bool correct)
        {
            if (correct)
        ...
        {
            if (BBEConfigs.OldStockfish)
            {
                OldEndGame(pm, chessBoard, correct);
                return;
            }
            if (correct)
            {
                int mateIn = puzzle.MateIn;
                CoreGameManager.Instance.AddPoints(mateIn * 100, 0, true);
                ItemMetaData[] items = ItemMetaStorage.Instance.FindAll(x => x.tags.Contains($"BBE_StockfishReward{mateIn}"));
                while (mateIn > 0 && items.Length == 0)
                {
                    mateIn--;
                    items = ItemMetaStorage.Instance.FindAll(x => x.tags.Contains($"BBE_StockfishReward{mateIn}"));
                }
                if (items.Length != 0)
                {
                    Pickup item = items.ChooseRandom().value.ToPickup();
                    item.icon = pm.ec.map.AddIcon(item.iconPre, item.transform, Color.white);
                }
            }
            puzzle = null;
            behaviorStateMachine.ChangeState(new StockfishWandering(this, 30));
