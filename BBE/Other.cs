...
namespace BBE
{
    ...
    public static class ExtensionsClasses
    {
        ...
        public static void DoIfElse<T>(this List<T> value, Func<T, bool> condition, Action<T> toDo, Action<T> toElse)
        {
            foreach (T val in value)
            {
                if (condition(val)) toDo(val);
                else toElse(val);
            }
        }
        public static SoundObject AddAdditionalKey(this SoundObject soundObject, string key, float time, bool encrypted = false)
        {
            if (soundObject.additionalKeys.IsNull()) soundObject.additionalKeys = new SubtitleTimedKey[] { };
        ...
        public static void EndGame(this CoreGameManager instance, Transform player, Transform targetPosition, UnityAction coroutinAction)
        {
            Time.timeScale = 0f;
            ...
            AudioManager audioManager = instance.audMan;
            WeightedSelection<SoundObject>[] loseSounds = ((Baldi)NPCMetaStorage.Instance.Get(Character.Baldi).value).loseSounds;
        ...
        public static List<T> ChooseRandom<T>(this IEnumerable<T> value, int count) => value.ToList().ChooseRandom(count);
        public static List<T> ChooseRandom<T>(this T[] value, int count) => value.ToList().ChooseRandom(count);
        public static List<T> ChooseRandom<T>(this List<T> value, int count)
        {
        ...
        public static T ChooseRandom<T>(this IEnumerable<T> value) => value.ToList().ChooseRandom();
        public static T ChooseRandom<T>(this T[] value) => value.ToList().ChooseRandom();
